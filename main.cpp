...
void func1(bool x){
    while(doNot){cout<<"test1";} //waiting doNot to be false (doing it in main function)
    ::fn2->join(); //trying to cause deadlock
}

void func2(bool x){
    while(doNot){cout<<"test2";} //waiting doNot to be false (doing it in main function)
    ::fn1->join(); //trying to cause deadlock
}

int main()
{
    //making threads for func1 and func2. I don't know why I've added arguements to funct1 & func2
    thread t1(func1,1);
    thread t2(func2,1);
